...
module.exports = app => {
    ...
    const get = (req, res) => {
        app.db('users').select('id', 'name', 'email', 'admin')
            .whereNull('deletedAt').then(users => res.json(users))
            .catch(err => res.status(500).send(err));
    }
    ...
    const getById = (req, res) => {
        ...
        if (!isNaN(req.params.id) && Number.isInteger(parseFloat(req.params.id))) {
            ...
            app.db('users').select('id', 'name', 'email', 'admin')
                .where({ id: userId }).whereNull('deletedAt')
                .first().then(user => res.json(user))
                .catch(err => res.status(500).send(err));
        } else {
            res.status(400).send('O Id informado é inválido');
        }
    }
