module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        if (category.id) {
            ...
            app.db('categories').insert(category).then(_ => res.status(204).send())
    ...
    const remove = async (req, res) => {
        try {
            ...
            notExistsOrError(articles, 'Categoia possui artigos.')
    ...
    const getTree = (req,res) => {
        app.db('categories').then(categories => res.json(toTree(withPath(categories))))
        .catch(err => res.status(500).send(err));
    }
